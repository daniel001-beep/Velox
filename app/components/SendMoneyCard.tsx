...
export default function SendMoneyCard({ onTransferSuccess }: SendMoneyCardProps) {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (newX >= maxSlide * 0.95 && !isProcessingRef.current) {
      setIsDragging(false);
  ...
  const initiateAtomicTransfer = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/ledger/transaction', {
        ...
        body: JSON.stringify({
          // Send as negative because it's an outbound transfer. Convert float to cents.
          amount: Math.round(-parseFloat(amount) * 100),
          idempotencyKey: crypto.randomUUID(),
          description: `Transfer to ${email} ${cleanNote ? `(${cleanNote})` : ''}`,
          metadata: { 
            type: 'TRANSFER', 
      ...
      setStatus('SUCCESS');
      setTxHash(data.transaction.hash || data.transaction.id);
      setShowConfetti(true);
      
      // Trigger callback to update ledger
      if (onTransferSuccess) {
        onTransferSuccess(data.transaction.id, email, amount, cleanNote);
      }
